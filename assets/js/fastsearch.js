...
// This script is loaded with `defer`, so the DOM is already parsed by the
// time it runs -- no need to wait for window's `load` event, which blocks
// on every subresource (images, fonts, etc.) and left the search box
// disabled and unresponsive to clicks/typing until everything else on the
