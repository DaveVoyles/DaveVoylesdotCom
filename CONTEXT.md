...
| Console theme | Light-first visual system: warm canvas, navy ink, green accent, monospace chrome. Tokens in `assets/css/extended/00-tokens.css`; grammar in `10-visual-grammar.css`; chrome in `custom.css`. | Prefer `--ds-*` (`--ds-ink`, `--ds-canvas`, `--ds-accent`) over hard-coded colors. Dark is optional, not default. |
